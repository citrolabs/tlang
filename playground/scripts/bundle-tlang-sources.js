...
/**
 * Extract top-level exports from tlang/src/index.ts
...
function bundleTLangSources() {
  console.log('📦 Bundling tlang sources...')

  // Read real package.json from tlang
  const tlangPackageJson = JSON.parse(readFileSync(TLANG_PACKAGE_JSON_PATH, 'utf-8'))
  console.log(`✅ Read package.json: ${tlangPackageJson.name}@${tlangPackageJson.version}`)

  // Extract top-level exports from index.ts
  const topLevelExports = extractTopLevelExports()
  console.log(`✅ Extracted ${topLevelExports.length} top-level exports`)
  ...
  // Generate package.json for VFS (using real data from tlang)
 ...
 *
 * Source: tlang@${tlangPackageJson.version}
...
try {
  ...
  console.error('❌ Failed to bundle tlang sources:', error)
  process.exit(1)
}
